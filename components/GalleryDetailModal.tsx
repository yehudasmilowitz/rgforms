...
export default function GalleryDetailModal({ module, onClose }: GalleryDetailModalProps) {
  const [tab, setTab] = useState<Tab>('client');
  const hasDetails = !!module.deploymentUrl;

  const result = { deploymentUrl: module.deploymentUrl ?? '' };
  const clientSnippet = hasDetails ? generateGalleryClientSnippet(result, module.moduleName) : '';
  const serverSnippet = hasDetails ? generateGalleryServerSnippet(result, module.moduleName) : '';
  const schemaSnippet = generateGallerySchemaSnippet(module.moduleName);
                ...
                {tab === 'client' && (
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                      Supports all, featured, category, and search queries. Responses cached 5 min.
                    </p>
                  </div>
                )}
                {tab === 'server' && (
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                      Fetch at build time or in a server component for fast initial page load.
                    </p>
                  </div>
                )}
                {tab === 'schema' && (
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                      Each Sheet row = one image. Add columns to extend the schema — no redeployment needed.
                    </p>
                  </div>
                )}
              </section>
