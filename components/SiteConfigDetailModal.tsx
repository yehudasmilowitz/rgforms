...
interface SiteConfigDetailModalProps {
  module: SiteConfigModuleSummary;
  onClose: () => void;
}

export default function SiteConfigDetailModal({ module, onClose }: SiteConfigDetailModalProps) {
  const [tab, setTab] = useState<Tab>('client');
  const hasDetails = !!module.deploymentUrl;

  const result = { deploymentUrl: module.deploymentUrl ?? '' };
  const clientSnippet = hasDetails ? generateConfigClientSnippet(result, module.moduleName) : '';
  const serverSnippet = hasDetails ? generateConfigServerSnippet(result, module.moduleName) : '';
  const schemaSnippet = generateConfigSchemaSnippet(module.moduleName);
              ...
              <section className="flex flex-col gap-4">
                ...
                {tab === 'client' && (
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                      Safe for public client-side code. Responses cached 5 min by default.
                    </p>
                  </div>
                )}
                {tab === 'server' && (
                ...
                {tab === 'schema' && (
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                      Add any row to the Config sheet to add a new key — no redeployment needed.
                    </p>
                  </div>
                )}
              </section>
