      ...
      <div className="w-full max-w-2xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ background: 'oklch(0.65 0.22 290 / 0.12)', border: '1px solid oklch(0.65 0.22 290 / 0.30)', color: 'oklch(0.72 0.18 290)' }}
            >
              <CheckIcon />
            </div>
            <div>
              ...
              <p className="text-sm mt-0.5" style={{ color: 'var(--color-muted)' }}>
                Your site config module is ready to use.
              </p>
            </div>
          </div>
          ...
          >
            {[
              'Google Sheet created with Config tab and starter keys',
              'Apps Script deployed as a public read API',
              'Keys are live-editable — no redeployment needed',
            ].map((item) => (
              <div key={item} className="flex items-center gap-2.5" style={{ color: 'oklch(0.65 0.22 290)' }}>
                <CheckIcon />
        ...
        >
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs leading-relaxed" style={{ color: 'var(--color-muted)' }}>
                Visit the endpoint URL once to authorize the script. After that it serves config to anyone automatically.
              </p>
            </div>
          </div>
          <a
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
            Append <code className="font-mono px-1 rounded" style={{ background: 'var(--color-surface-2)' }}>?json=1</code> for JSON — the bare URL shows an authorization confirmation page.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {tab === 'client' && (
            <div className="flex flex-col gap-3">
              <CopyBlock label="RGConfig.js" content={clientSnippet} language="js" />
              <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                Safe for public client-side code — read-only, no secrets. Responses cached 5 min by default.
              </p>
            </div>
          )}
          {tab === 'server' && (
          ...
          {tab === 'schema' && (
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                Add any key-value pair to the Config sheet and it appears in the API immediately.
              </p>
            </div>
          )}
        </div>
